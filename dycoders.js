//RECODE? MINIMAL KASIH CREADIT ©dycoders.xyz
 //DI KEMBANGKAN OLEH dycoders.xyz
 //GKK KASIH CREADIT YATIM YAPIT
...
const { venice } = require('./lib/scrape/venice.js'); 
...
if (m.message) {
if (isCmd && !m.isGroup) {
console.log(chalk.black(chalk.bgHex('#9CA3AF').bold(`\n🌟 ${ucapanWaktu} 🌟`)));
console.log(chalk.white(chalk.bgHex('#4a69bd').bold(`HALLO OWNER! ADA PESAN NIH`)));
console.log(chalk.black(chalk.bgHex('#FACC15')(`📅 DATE: ${new Date().toLocaleString()}
...
if (chats.antipromosi && !m.key.fromMe && !isDycoders && !isAdmins && isBotAdmins) {
...
if (chats.antilink && !m.key.fromMe && !isDycoders && !isAdmins && isBotAdmins) {
...
case 'sendemail': {
    ...
    if (!text.includes('|')) return m.reply(`Format salah! Contoh: .sendemail email|subject|text`);
    
    const [email, subject, body] = text.split('|');
    if (!email || !subject || !body) return m.reply("Semua parameter (email, subject, text) harus diisi!");

    const encodedEmail = encodeURIComponent(email.trim());
    const encodedSubject = encodeURIComponent(subject.trim());
    const encodedBody = encodeURIComponent(body.trim());

    const url = `https://api.catozolala.store/send-email?penerima=${encodedEmail}&subject=${encodedSubject}&text=${encodedBody}`;

    try {
        const res = await fetch(url);
        const json = await res.json();

        if (json.message === "Email berhasil dikirim") {
            m.reply(`✅ Email berhasil dikirim!\n\n📧 Penerima: ${email}\n📝 Subjek: ${subject}\n✉️ Isi: ${body}`);
        } else {
            m.reply(`❌ Gagal mengirim email: ${json.message}`);
        }
    } catch (error) {
...
case 'venice': {
    ...
    if (command === 'text') {
        try {
            ...
            const answer = await venice.chatbot(userInput, model); 
            await dy.sendMessage(m.chat, { text: answer }); 
        } catch (error) {
            console.error('Error generating text:', error);
            m.reply("⚠️ Terjadi kesalahan saat mendapatkan jawaban.");
        }
    } else if (command === 'image') {
       
        try {
            const imagePrompt = userInput; 
            const result = await venice.txt2img(imagePrompt); 
            await dy.sendMessage(m.chat, { image: { url: './media/image.png' }, caption: "Gambar berhasil dibuat!" }); 
        } catch (error) {
            console.error('Error generating image:', error);
            m.reply("⚠️ Terjadi kesalahan saat membuat gambar.");
        }
    } else {
        m.reply('⚠️ Perintah tidak dikenali. Gunakan "text" untuk teks atau "image" untuk gambar.');
    }
    break;
}
...
`);
    ...
    try {
        ...
        while (message.length > maxLength) {
            ...
            await dy.sendMessage(result, { text: chunk }, { quoted: fverif });
            message = message.substring(maxLength);
        }

        if (message.length > 0) {
            await dy.sendMessage(result, { text: message }, { quoted: fverif });
        }

        await dy.sendMessage(m.chat, { react: { text: '⌛', key: m.key } });

        if (global.responseListener) {
            dy.ev.off('messages.upsert', global.responseListener);
        }
        ...
        dy.ev.on('messages.upsert', global.responseListener);
    } catch (e) {
        console.error(e);
...
case 'antipromosi':
    ...
    case 'delcase': {
    if (!isDycoders) return reply(`*Access Denied ❌*\n\n*Owners only*`)
...
case 'setbotname':{
if (!isDycoders) return reply(`Khusus dycoders.xyz`)
...
Tidak bisa ditarik atau transfer!\n`
    ...
    break
case 'report': {
  if (!text) return m.reply(`Mau report error apa ke developer?\n\nAnda juga bisa mengirim bukti error.`);

  const owner = "6285719898124";

  if (/image/.test(mime)) {
    if (!quoted) return reply(`Reply image`);
    if (!text) return reply(`Mau report error apa ke developer?`);

    let img = await quoted.download();
    let media = await uploadImage(img);
    m.reply(`Terimakasih atas masukan Anda, report Anda akan disampaikan ke developer.`);

    dy.sendImage(owner + "@s.whatsapp.net", media, `*[ System Notice ]* Ada yang report error nih, dari @${m.sender.split("@")[0]}\nPesan : *${text}*\n`, m);
  } else {
    m.reply(`Terimakasih atas masukan Anda, report Anda akan disampaikan ke developer.`);
    dy.reply(owner + "@s.whatsapp.net", `*[ System Notice ]* Ada yang report error nih, dari @${m.sender.split("@")[0]}\nPesan : *${text}*\n`, m);
  }
}
break;
...
case 'autodl': {
    if (!isDycoders && !isPremium) return reply(mess.premium)
...
break;
case 'vps': {
    if (!isDycoders) return m.reply("BUKAN DY");
...
const path = require('path');
    if (!text) {
        return m.reply('Masukkan URL Mediafire!\nContoh: !mediafire https://www.mediafire.com/file/example');
    }
    ...
    try {
        ...
        const url = text.trim();
        const apiUrl = `https://restapi.apibotwa.biz.id/api/mediafire?url=${encodeURIComponent(url)}`;

        const response = await axios.get(apiUrl);
        const data = response.data;

        if (data.status !== 200 || !data.data || !data.data.response) {
            return m.reply('⚠️ Gagal mengambil data dari Mediafire. Pastikan link yang Anda masukkan benar.');
        }

        const fileInfo = data.data.response;
        const downloadLink = fileInfo.download;
        ...
        writer.on('finish', async () => {
            const message = `📂 *File Name*: ${fileInfo.filename}\n📄 *Type*: ${fileInfo.type}\n💾 *Size*: ${fileInfo.size}`;
        ...
        writer.on('error', (err) => {
            console.error('[ERROR] Gagal mendownload file:', err.message);
            m.reply('⚠️ Gagal mendownload file. Terjadi kesalahan saat mengambil file dari Mediafire.');
        });

    } catch (error) {
        console.error('[ERROR] Terjadi kesalahan saat memproses Mediafire URL:', error.message);
        console.error(error.stack);
        m.reply('⚠️ Terjadi kesalahan saat memproses permintaan. Pastikan URL Mediafire valid.');
    }
    break;
}
...
case 'joinresellerpanel': {
  ...
  const amount = global.hargareseller;
  ...
  const cekTrxApiUrl = `https://gateway.okeconnect.com/api/mutasi/qris/${global.Merchantid}/${global.ApiOrkut}`;
  ...
  try {
    ...
    const databasePath = './lib/database/databaseorkut.json';
    let database = JSON.parse(fs.readFileSync(databasePath));

    while (!isPaymentSuccessful && Date.now() - startTime < timeout) {
      try {
        await new Promise((resolve) => setTimeout(resolve, 15000));
        const cekTrxResponse = await fetch(cekTrxApiUrl, {
          headers: {
            "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36",
          },
        });
        if (!cekTrxResponse.ok) throw new Error('⚠️ Gagal memeriksa transaksi.');
        const cekTrxData = await cekTrxResponse.json();

        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          const latestTransaction = cekTrxData.data.find(
            (trx) =>
              trx.amount === String(amount) &&
              trx.type === "CR" &&
              !database.processed_transactions.includes(trx.issuer_reff)
          );

          if (latestTransaction) {
            isPaymentSuccessful = true;

            database.processed_transactions.push(latestTransaction.issuer_reff);
            fs.writeFileSync(databasePath, JSON.stringify(database, null, 2));

            await m.reply(
    `✅ Pembayaran berhasil!\n\n🗓️ *Tanggal*: ${latestTransaction.date}\n💵 *Jumlah*: ${latestTransaction.amount}\n🏷️ *Brand*: ${latestTransaction.brand_name}\n🔄 *Tipe*: ${latestTransaction.type}\n\n*Berikut Link Grub Reseller Panel:*\n${global.linkreseller}\n\n\`\`\`Mohon Tunggu 5-10 Menit Agar Di Acc Permintaan Join Anda dan Add ID Reseller ke Database nya.\`\`\`\n\`\`\`Nb: Jika dalam 5-10 Menit belum ada respon berarti sedang Sibuk, Mohon Bersabar.\`\`\``
  );
}
}

      } catch (error) {
        console.error("⚠️ Kesalahan saat memeriksa transaksi:", error.message);
      }
    }

    if (!isPaymentSuccessful) {
      m.reply("⚠️ Deposit gagal! Tidak ada transaksi terdeteksi dalam waktu 10 menit.");
    }
  } catch (error) {
    console.error(error);
    m.reply("⚠️ Terjadi kesalahan saat membuat QR code atau memeriksa transaksi.");
  }
  break;
}
//subdomain
    ...
    if (!isPremium) return reply("Kamu Belum Menjadi User Premium Silahkan Beli Premium Ke Owner Dengan Ketik .owner")
           function subDomain1(host, ip) {
             return new Promise((resolve) => {
               ...
               let apitoken = global.apikeysubdo
               let tld = global.subdomain;
               axios
                 .post(
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const eggnya = global.eggnya;
            const locnya = global.locnya;
            const inputNumber = m.sender.split('@')[0];
            const username = `user_${crypto.randomBytes(3).toString('hex')}_${inputNumber}`;
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 1GB - Aktif 1 Bulan`;
            ...
            try {
              ...
              const panelData = `*DATA PANEL ANDA*\n\n👤 Username: ${user.username}\n🔐 Password: ${password}\n🌐 URL: ${domain}\n📋 Deskripsi: ${deskripsi}`;
              await dy.sendMessage(m.chat, { text: panelData });
            } catch (err) {
              console.error(err);
              m.reply("⚠️ Gagal membuat panel.");
            }
          }
        }
      } catch (error) {
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 2GB - Aktif 1 Bulan`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 3GB - Aktif 1 Bulan`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 4GB - Aktif 1 Bulan`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 5GB - Aktif 1 Bulan`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 6GB - Aktif 1 Bulan`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server 7GB - Aktif 1 Bulan`;
...
case 'buyadminpanel': case 'buyadp': {
  ...
  const amount = global.adminpanel;
  const qrisApiUrl = `https://skizoasia.xyz/api/qris-converter?apikey=${global.skizo}&url=${global.qris}&amount=${amount}&hasService=false&serviceMode=p&serviceAmount=0`;
  const cekTrxApiUrl = `https://gateway.okeconnect.com/api/mutasi/qris/${global.Merchantid}/${global.ApiOrkut}`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server unli+ADMIN PANEL - Aktif 1 Bulan\n\n ${global.linkgrupadminpanel}`;
  ...
  async function cekTransaksi() {
    try {
      ...
      if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
        ...
        cekTrxData.data.forEach((trx) => {
          if (
            trx.type === "CR" &&
            !database.processed_transactions.includes(trx.issuer_reff)
          ) {
            database.processed_transactions.push(trx.issuer_reff);
            fs.writeFileSync(databasePath, JSON.stringify(database, null, 2));

            console.log(`✅ Transaksi baru terdeteksi:\nID: ${trx.issuer_reff}, Jumlah: ${trx.amount}`);
            newTransactions++;
          }
        });

        if (newTransactions > 0) {
          m.reply(`✅ ${newTransactions} transaksi baru berhasil dimasukkan ke database.`);
        } else {
          m.reply('ℹ️ Tidak ada transaksi baru yang ditemukan.');
        }
      } else {
        m.reply('⚠️ Tidak ada data transaksi yang tersedia.');
      }
    } catch (error) {
      console.error("⚠️ Kesalahan saat memeriksa transaksi:", error.message);
      m.reply("⚠️ Terjadi kesalahan saat memeriksa transaksi.");
    }
  }
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server unli - Aktif 1 Bulan`;
...
case 'buyownerpanel': {
  ...
  const amount = global.ownerpanel;
  const qrisApiUrl = `https://skizoasia.xyz/api/qris-converter?apikey=${global.skizo}&url=${global.qris}&amount=${amount}&hasService=false&serviceMode=p&serviceAmount=0`;
  const cekTrxApiUrl = `https://gateway.okeconnect.com/api/mutasi/qris/${global.Merchantid}/${global.ApiOrkut}`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server unli+ OWNER PANEL - Aktif 1 Bulan  \n\n ${global.linkownerpanel}`;
      ...
      try {
        ...
        if (cekTrxData.status === "success" && Array.isArray(cekTrxData.data)) {
          ...
          if (latestTransaction) {
            ...
            const email = `${username}@dycoders.xyz`;
            const password = crypto.randomBytes(5).toString('hex');
            const deskripsi = `Server unli+PT PANEL - Aktif 1 Bulan\n\n ${global.linkptpanel}`;
...
case 'getpb': {
  ...
  if (!link) return m.reply('Mana link Pastebin-nya?');
  if (!/^https:\/\/pastebin\.com\/[a-zA-Z0-9]+$/.test(link)) return m.reply('URL tidak valid.');
  ...
  try {
    ...
    const result = content.length > maxMessageLength 
      ? content.slice(0, maxMessageLength) + `\n\n[Pesan dipotong karena melebihi ${maxMessageLength} karakter.]` 
      : content;
...
case 'searchapp': {
    if (!text) return reply(`*Example*: ${prefix + command} PUBG`);
    
    try {
        const { searchApp } = require('./lib/scrape/dva');
        const results = await searchApp(text);
        
        if (results.length === 0) return reply(`Tidak ditemukan hasil untuk: *${text}*`);

        let message = `🔍 *Hasil Pencarian untuk: ${text}*\n\n`;
        results.forEach((res, i) => {
            message += `*${i + 1}. ${res.title}*\n`;
            message += `🔗 Link: ${res.link}\n`;
            message += `📥 Version: ${res.version}\n\n`;
        });
...
break;
case 'downloadapp': {
    if (!text) return reply(`*Example*: ${prefix + command} https://gamedva.com/pubg`);

    try {
        const { getDownloadInfo } = require('./lib/scrape/dva'); 
        const result = await getDownloadInfo(text); 

        if (!result.link) return reply(`Tidak ditemukan link download untuk URL tersebut.`);

        let message = `🎮 *Informasi Download*\n\n`;
        message += `*Judul*: ${result.detail.title || 'Tidak Diketahui'}\n`;
        message += `*Deskripsi*: ${result.detail.description || 'Tidak Tersedia'}\n`;
        message += `*Author*: ${result.detail.author || 'Tidak Diketahui'}\n`;
        message += `*Link*: ${result.detail.links || 'Tidak Tersedia'}\n\n`;
        message += `📥 *Download*: ${result.link}\n`;
        message += `ℹ️ *Info*: ${result.info || 'Tidak Ada Info'}\n`;

        reply(message);
    } catch (err) {
        console.error(err);
        reply(`Terjadi kesalahan saat mengambil informasi download.`);
    }
}
break;
...
case 'pixiv': {
    if (limitnya < 1) return m.reply(mess.limit);
    if (!text) return reply(`*Example*: ${prefix + command} Gojo Satoru --r18`);
    
    const r18 = text.includes("--r18"); 
    const keyword = text.replace("--r18", "").trim();
    
    if (!keyword) return reply(`Keyword tidak valid! Gunakan format: ${prefix + command} <keyword> [--r18]`);
    ...
    try {
        const pixiv = require('./lib/scrape/pixiv'); 
        const results = await pixiv(keyword, r18);
        
        if (results.length === 0) return reply(`Tidak ada hasil untuk keyword: *${keyword}*`);
        ...
        for (let res of results.slice(0, 5)) {
            push.push({
                ...
                header: proto.Message.InteractiveMessage.Header.fromObject({
                    title: res.title,
                    hasMediaAttachment: true,
                    imageMessage: await createImage(res.images)
                }),
                nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                    buttons: [
                        {
                            name: "cta_url",
                            buttonParamsJson: `{"display_text":"Lihat Gambar","Klik disini":"${res.images}","merchant_url":"${res.images}"}`
                        }
                    ]
                })
            });
        ...
        console.error(err);
        reply(`Terjadi kesalahan saat mengambil data dari API Pixiv.`);
    }
}
break;
...
case 'sendmsg': {
    ...
    let [msg, number] = text.split(',');
    if (!msg || !number) return m.reply(`Format salah! Contoh: .sendmsg halo,628xxxxxxx`);
    
    const target = number.replace(/[^0-9]/g, '') + '@s.whatsapp.net';

    try {
        await dy.sendMessage(target, { text: msg });
        m.reply(`Pesan berhasil dikirim ke ${number}`);
    } catch (error) {
        console.error(error);
        m.reply(`Gagal mengirim pesan ke ${number}. Periksa nomor atau koneksi.`);
    }
    break;
}
...
case 'sendmedia': {
    ...
    if (!m.quoted) return m.reply("Balas media yang ingin dikirim dengan perintah ini.");
    if (!args[0]) return m.reply("Masukkan nomor tujuan! Contoh: .sendmedia 628xxxxxxx");
    ...
    try {
        ...
        if (/image/.test(mime)) {
            await dy.sendMessage(target, { image: media, caption: "Gambar ini telah dikirim!" });
        } else if (/video/.test(mime)) {
            await dy.sendMessage(target, { video: media, caption: "Video ini telah dikirim!" });
        } else if (/audio/.test(mime)) {
            await dy.sendMessage(target, { audio: media, mimetype: 'audio/mp4' });
        } else {
            return m.reply("Media yang didukung hanya gambar, video, dan audio!");
        }

        m.reply(`${args[0]}`);
    } catch (error) {
        console.error(error);
        m.reply(`Gagal mengirim media ke ${args[0]}. Periksa nomor atau koneksi.`);
    }
    break;
}
...
case 'acc': {
    if (!m.isGroup) return m.reply(mess.group)
    if (!isDycoders && !isAdmins) return m.reply(mess.admin)
...
if (!isAdmins && !isDycoders) return reply("Khusus Admin Group")
...
case 'askgpt': {
    ...
    try {
        const response = await fetch(`https://server.apisanz.my.id/ai/chatgpt?text=${encodeURIComponent(text)}`);
...
if (!isAdmins && !isDycoders) return reply(mess.admin)
...
if (!isAdmins && !isDycoders) return m.reply(mess.admin)
if (/image/g.test(mime)) {
...
case 'welcome': {
if (!isDycoders) return reply('Ngapain ? Fitur Ini Khusus Tuan Saya😜')
...
if (!from) return reply(`Kirim perintah *${command}* @tag atau reply pesan orangnya!`)
if (from === botNumber) return reply(`Tidak bisa bermain dengan bot!`)
if (from === sender) return reply(`Sad amat main ama diri sendiri`)
var hadiah = randomNumber(10, 20)
await reply(`@${sender.split('@')[0]} menantang @${from.split('@')[0]} untuk bermain TicTacToe\n\n*Kirim (Y/N)* untuk bermain\n\nHadiah : ${hadiah} balance`)
tictactoe[from] = {
id: from,
status: null,
hadiah: hadiah,
