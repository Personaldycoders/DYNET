...
class Button {
    ...
    addView(link = "", success_url = "", cancel_url = "") {
          this._beton.push({
              name: "open_webview",
              buttonParamsJson: JSON.stringify({
                  link,
    ...
    async run(jid, dy, quoted = {}) {
        const message = {
            ...
            header: proto.Message.InteractiveMessage.Header.create({ title: this._title, subtitle: this._subtitle, hasMediaAttachment: (this._data ? true : false), ...(this._data ? await prepareWAMessageMedia(this._data, { upload: dy.waUploadToServer }) : {}) })
        };
        ...
        await dy.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
        return msg;
    }
}
